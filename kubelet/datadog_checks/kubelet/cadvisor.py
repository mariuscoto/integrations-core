...
# check
from .common import tags_for_container, tags_for_docker, tags_for_pod, is_static_pending_pod, get_pod_by_uid
        ...
        if is_pod:
            ...
            cid = pod_list_utils.get_cid_by_name_tuple(
                (pod.get('metadata', {}).get('namespace', {}),
                 pod.get('metadata', {}).get('name', {}), k_container_name), True)
            if pod_list_utils.is_excluded(cid):
                self.log.debug("Filtering out " + cid)
                return
            tags = tags_for_container(cid, True)
